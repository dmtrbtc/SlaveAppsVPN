...
async function ready() {
  for (let i = 0; i < 40; i++) {
    if (await evaluate('!!window.slaveVPN?.settings')) return
    await pause(200)
  }
...
const verify = `(async () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (native?.language === 'en' && native?.dnsPreset === 'balanced') break;
    await new Promise(r => setTimeout(r, 100));
  }
  const valid = s => s?.language === 'en' && s?.dnsPreset === 'balanced' &&
...
async function main() {
  ...
  const preflight = await evaluate(`(async () => {
    const read = async key => (await window.Capacitor.nativePromise('Preferences', 'get', {key})).value;
    const {status} = await window.Capacitor.nativePromise('SlaveVpn', 'getStatus', {});
    return {
      state: status.state,
      localSettings: localStorage.getItem('slave.settings.v1'),
      nativeSettings: await read('slave.settings.v1'),
      localSubscriptions: localStorage.getItem('slave.subscriptions.index.v1'),
      nativeSubscriptions: await read('slave.subscriptions.index.v1'),
    };
  })()`)
  assertFreshSmokeTarget(preflight)
  freshTargetClaimed = true
  const writes = await evaluate(`(async () => {
    const result = await Promise.all([
      window.slaveVPN.settings.set({language:'ru',enabledScenarios:['roscomvpn-default']}),
      window.slaveVPN.settings.set({language:'en',dnsPreset:'balanced',enabledScenarios:['ai-services']})
    ]);
...
main().catch(error => { console.error(error.message); process.exitCode = 1 }).finally(async () => {
  await close()
  if (freshTargetClaimed) adb('shell', 'am', 'force-stop', packageName)
})
