...
**Кроссплатформенный VPN-клиент (Android и Windows) на ядре mihomo (Clash.Meta).**

Подключение по подписке (Remnawave), гибкая маршрутизация трафика, поддержка VLESS-Reality с шифрованием, Hysteria2, Trojan и других протоколов.
...
- **Гибкая маршрутизация:** часть трафика идёт напрямую, часть — через VPN, по правилам. Режимы: **Умный / Глобальный / Прямой**.
- **Внешние списки правил** маршрутизации (домены из публичных списков) с ручным обновлением по кнопке.
...
- **Защищённый DNS** (DoH через туннель, без утечек; локальные домены резолвятся напрямую для скорости).
...
- **Hysteria2 = QUIC/UDP** — у некоторых операторов QUIC/UDP может ограничиваться DPI; VLESS-Reality остаётся основным рабочим протоколом.
...
**[⬇️ Скачать последнюю альфу](https://github.com/dmtrbtc/SlaveAppsVPN/releases/tag/v0.2.0-alpha.1)** · сделано с ❤️
